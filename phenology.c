// Description: Dynamics of a mutualistic evological system
...
#define networks 1 // number of different networks
...
void readBipartiteMatrix (char* networkName); // read bipartite matrix file
...
void globalVariablesInitialization (int argc, char **argv)
{
    ...
    if (argc != 16)
    {
      printf ("\nError: there are not 15 parameters.\n");
      exit (-1);
    }
	...
	betaP_ii_0=1.*atoi(argv[5])/100; // plants competition constant (intra-specie)
	min_gammaP_0=1.*atoi(argv[6])/100; // plants mutualism constant, minimum value to sweep the interval
	...
	min_alphaP=1.*atoi(argv[9])/100; // plants growing constant
	max_alphaP=1.*atoi(argv[10])/100; // animals growing constant
	hP=1.*atoi(argv[11])/100; // holling term for plants (constant) ; ~(N_A*gammaP_0)?
    rewiringsBeforeFirstNetwork = atoi(argv[12]);
 	competitionMeanField = atoi(argv[13]);
	asymmetric = atoi(argv[14]);
	phenology = atoi(argv[15]);
...
{
	int biodiversity1, biodiversity2, i;
	...
	readBipartiteMatrix (argv[1]);
	if (nullModelNetwork)
		nullModel ();
	for (rewiringNumber=0; rewiringNumber < rewiringsBeforeFirstNetwork; rewiringNumber++)
		nestednessNODF = swap ();
	nestedness = largestEigenvalue (N_P+N_A);
	for (networkNumber=0; networkNumber<networks /* *rewiringsPerSwitchOfNetwork*/; networkNumber++)
	{
		// printProjectedMatrices (argv [1], networkNumber);
		nestedness = largestEigenvalue (N_P+N_A);
		nestednessNODF = NODF ();
		printf ("largest eigenvalue=%f, nestednessNODF=%f\n", nestedness, nestednessNODF);
		for (betaP_0=min_betaP_0; betaP_0 < max_betaP_0+0.0001; betaP_0 += delta_betaP_0) // plants competition constant (cross-species)
		{
			betaA_0 = betaP_0;// inicialited by symmetry (plants constant = animal constant)
			for (gammaP_0=min_gammaP_0; gammaP_0 < max_gammaP_0+0.0001; gammaP_0 += delta_gammaP_0)  // plants mutualism constant
			{
				gammaA_0 = gammaP_0; // inicialited by symmetry (plants constant = animal constant)
				initialConditions ();
				equationDynamics ();
				printAbundancesVector (nestedness, nestednessNODF, biodiversity(),  networkNumber);
			}
		}
		/*for (rewiringNumber=0; rewiringNumber < rewiringsPerSwitchOfNetwork; rewiringNumber++)
			rewiring ();*/
		nestednessNODF = swap ();
		nestedness = largestEigenvalue (N_P+N_A);
	}
	/*	// network dynamics through rewiring:
	for (i=0; i<(N_P+N_A)*10; i++) // randomize the network:
		rewiring ();
	equationDynamics ();
	biodiversity1 = biodiversity ();
	do
	{
		printf ("networkNumber=%d, N_P+N_A=%d, biodiversity=%d\n", re++,  N_P+N_A, biodiversity1);
		r=rewiring ();
		nestedness= largestEigenvalue (N_P+N_A);
		equationDynamics ();
		biodiversity2 = biodiversity ();
		if (biodiversity1 < biodiversity2) // accept rewiring
			biodiversity1 = biodiversity2;
		else
		{
			undoRewiring (r);
			nestedness = largestEigenvalue (N_P+N_A);			
